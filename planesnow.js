...
function update() {
  ...
  snowflakes.forEach(s => {
    ...
    else if (s.x < 0) s.x = canvas.width;
  });
}
...
function draw() {
  ...
  // Blast cooldown display top-right
  ctx.textAlign = "right";
  ctx.fillStyle = blastCooldown > 0 ? "red" : "green";
  const cdSeconds = (blastCooldown / 60).toFixed(1);
  ctx.fillText(`Blast Cooldown: ${blastCooldown > 0 ? cdSeconds + "s" : "Ready"}`, canvas.width - 10, 20);
  ...
  if (gameOver) {
    ...
    ctx.font = "24px Arial";
    ctx.textAlign = "center";
    ctx.fillText(endMessage, canvas.width / 2, canvas.height / 2 - 40);
