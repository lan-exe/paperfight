...
// Game state
let keys = {}, enemies = [], bullets = [], particles = [], clouds = [], perks = [], winds = [], rainDrops = [];
...
const player = {
  ...
  acc: 0.15, drag: 0.98, rotSpeed: 0.04,
  health: 100, shootDelay: 0
};
...
function draw() {
  ...
  ctx.textAlign = "right";
  if (blastCooldown > 0)
    ctx.fillText(`Blast Cooldown: ${(blastCooldown / 60).toFixed(1)}s`, canvas.width - 10, 10);
  else
    ctx.fillText(`Blast Ready!`, canvas.width - 10, 10);
